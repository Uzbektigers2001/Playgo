...
### Phase 5: Final Polish (Prompt 10)
- **Database seeder** (`DbSeeder` in `Playgo.Infrastructure.Persistence.Seeders`) — idempotent seed of admin user, 12 default genres (Action, Drama, Comedy, Sci-Fi, Horror, Thriller, Romance, Animation, Documentary, Adventure, Crime, Fantasy), and 3 subscription plans for every environment. 10 demo Movies/Series only seed in Development or Staging — production never gets fixture content. Admin email comes from `Seed:AdminEmail` (default `admin@playgo.uz`); password reads `SEED_ADMIN_PASSWORD` env var first, then `Seed:AdminPassword`, then `"Admin123!"`. Wired into `Program.cs` after auto-migrate, before `app.Run()`.
- **Full Swagger XML documentation** — `GenerateDocumentationFile=true` enabled on every csproj; `SwaggerExtensions.AddSwaggerWithJwt` now `IncludeXmlComments` for `Playgo.API`, `Playgo.Application`, and `Playgo.Domain`. Every controller has a class-level `[Tags("...")]` (Auth, Contents, Movies (Legacy), Genres, Reviews, Favorites, Watchlist, Playlists, Watch History, Upload, Plans, Subscriptions, Payments, Admin - Users / Contents / Reviews / Plans / Dashboard) and a class-level `<summary>`. `AuthController` actions carry full per-action XML comments and `[ProducesResponseType]` attributes covering 200 / 201 / 204 / 400 / 401 status codes.
- **Postman API collection** at `docs/api/Playgo.postman_collection.json` — 18 folders mirroring the Swagger tags. `Login` request has a test script that writes `accessToken` and `refreshToken` into the Postman environment so subsequent Bearer-auth requests just work. Companion `docs/api/local.postman_environment.json` ships with `baseUrl`, `adminEmail`, `adminPassword` and placeholders for IDs.
- **GitHub Actions CI** at `.github/workflows/ci.yml` — boots PostgreSQL 16 and Redis 7 services, restores, builds Release, runs `dotnet test --collect:"XPlat Code Coverage" --logger trx`, publishes a test report and uploads the cobertura coverage XML as an artifact (`code-coverage`, 14 days). Docker build smoke check runs after tests pass.
- **Docker image workflow** at `.github/workflows/docker.yml` — pushes to `ghcr.io/<repo>/playgo-api` on `main` (tagged `latest` + short sha) and on `v*` tags (semver tags). Uses Docker Buildx with GHA cache.
- **Smoke test HTTP file** at `docs/smoke-test.http` — VS Code REST Client format; 15 sequential calls covering register → login → me → contents → favorites → watchlist → reviews → vote → plans → subscribe → callback → mine → health probes.
- **Integration tests** with `WebApplicationFactory<Program>` — new `tests/Playgo.Tests/Common/TestWebAppFactory.cs` swaps EF Core for InMemory, stubs `ICacheService` with a no-op, and skips Hangfire/Redis registration in the `Testing` host environment so tests run without Docker. Hangfire dashboard mount and recurring job setup are now gated on `!IsEnvironment("Testing")`.
- **Test data builders** with Bogus — `tests/Playgo.Tests/Common/TestDataBuilder.cs` provides `CreateUser`, `CreateGenre`, `CreateContent`, `CreateReview`, `CreatePlan`.
- **Coverage collection enabled** — Coverlet bumped to 6.0.2; `.csproj` documentation builds cleanly with CS1591 suppressed.
- **+34 tests added across this prompt** (45 → ~92 total): `AuthServiceTests` (11), `ContentServiceTests` (6), `FavoriteServiceTests` (5), `WatchHistoryServiceTests` (5), `ReviewServiceTests` (4), `Integration/AuthEndpointsTests` (7), `Integration/ContentsEndpointsTests` (4), `Integration/AdminEndpointsTests` (3 — including 401-no-token, 403-non-admin, 200-seeded-admin paths).
...
- 18 new unit tests across `CacheServiceTests`, `EmailServiceTests`, `EmailVerificationFlowTests`, `PasswordResetFlowTests`, `RefreshTokenRotationTests` (replay detection, logout-all, single-session logout).
